...
import { Badge, Btn, DataCard, EmptyState, Modal, PageToolbar, SectionCard, inputCls } from "@/lib/list-utils";
import { decideRbacApproval } from "@/lib/rbac/rbac.functions";
import { Field, KeyValue, formatRiyadh, remainingLabel, staffName, type RbacApproval, type RbacOverview } from "./shared";

const STATUS: Record<RbacApproval["status"], { label: string; tone: "gold" | "green" | "red" | "muted" | "info" }> = {
...
function DiffView({ payload }: { payload: Record<string, unknown> | null }) {
  ...
  if (!before && !after) {
    return (
      <pre dir="ltr" className="max-h-56 overflow-auto rounded-[var(--radius-m)] bg-surface-muted p-3 text-[11px]">
      ...
      <table className="w-full min-w-[420px] text-right text-[12px]">
        ...
        <tbody>
          {keys.map((k) => {
            ...
            return (
              <tr key={k} className={b === a ? "border-t border-border" : "border-t border-border bg-warning-soft/40"}>
...
export function ApprovalsPanel({
  ...
  const [review, setReview] = useState<{ req: RbacApproval; decision: "approved" | "rejected"; reason: string } | null>(
    null,
  );
  ...
  const decide = useMutation({
    mutationFn: () => decideFn({ data: { id: review!.req.id, decision: review!.decision, reason: review!.reason } }),
    onSuccess: () => {
  ...
  const mine = data.approvals.filter((a) => a.status === "pending" && a.requested_by === data.me.userId).length;

  return (
    <div className="space-y-5">
      <SectionCard title="مبدأ أربع أعين" description="كل عملية حساسة تتطلب طلباً واعتماداً من شخصين مختلفين.">
        <div className="grid gap-3 sm:grid-cols-3">
          <KeyValue label="طلبات معلّقة">{data.approvals.filter((a) => a.status === "pending").length}</KeyValue>
      ...
      <div>
        ...
        {rows.length === 0 ? (
          <DataCard>
            <EmptyState title="لا توجد طلبات مطابقة" hint="الطلبات الحساسة تظهر هنا لاعتمادها أو رفضها." />
          </DataCard>
        ) : (
                ...
                <li key={a.id} className="surface-card p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="font-semibold">
                        <span className="font-mono text-[12px]">{a.action}</span> · {a.resource_type}
                      </p>
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-4">
                    ...
                    <KeyValue label="ينتهي">
                      ...
                      {a.status === "pending" && !expired && ` — ${remainingLabel(a.expires_at, now)}`}
                    </KeyValue>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    ...
                    {a.status === "pending" && !expired && (
                      <>
                        ...
                        {self && <span className="text-[12px] text-warning">أنت مقدّم الطلب — يعتمده زميل آخر.</span>}
                      </>
                    )}
                  </div>
                  {a.decision_reason && <p className="text-caption mt-2">تعليل القرار: {a.decision_reason}</p>}
                </li>
              );
            })}
          </ul>
