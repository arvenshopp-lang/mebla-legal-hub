 ...
 * المساعد القانوني الذكي العائم والمتوافق بنسبة 100% مع الجوال والشاشات المختلفة
 * يراعي عدم حجب أزرار التنقل السفلية (Safe Margins) وتجربة اللمس الفائقة
 * ==============================================================================
 */
...
يمكنك اختيار قضية محددة من القائمة العلوية أو سؤالي مباشرة عن أي مسألة في قضاياك.`,
  ...
  if (!activeOrgId || pathname === "/bayan") return null;

  return (
    <>
      {/* الزر العائم الدائم — يرتفع فوق شريط التنقل السفلي في الجوال (bottom-20) */}
      {!isOpen && (
        <button
          onClick={() => {
            setIsOpen(true);
          ...
          className="fixed bottom-20 left-4 sm:bottom-6 sm:left-6 z-30 flex items-center gap-2 rounded-full bg-gradient-to-r from-[#123C32] to-[#1E5648] px-3.5 py-2.5 sm:px-4 sm:py-3 text-white shadow-xl hover:brightness-110 hover:scale-105 active:scale-95 transition-all group border border-[#C9A961]/40"
          title="استشارة المحامية بيان"
          aria-label="المحامية بيان"
        >
          <div className="relative flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center rounded-full bg-[#123C32] text-[#C9A961]">
            <Sparkles className="h-3.5 w-3.5 sm:h-4 sm:w-4 animate-pulse" />
            <span className="absolute -top-0.5 -right-0.5 flex h-2 w-2 sm:h-2.5 sm:w-2.5 rounded-full bg-emerald-400 ring-2 ring-[#123C32]" />
          </div>
          <span className="text-xs sm:text-sm font-bold tracking-wide">المحامية بيان</span>
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-[#E6E2D8] dark:border-[#2A3632] bg-white dark:bg-[#17201D] px-4 py-3 shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-[#5F6B66] dark:text-[#8A9892] max-w-[200px] sm:max-w-[260px] truncate">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="hidden sm:flex h-8 w-8 items-center justify-center rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
          ...
          {!isMinimized && (
            <>
              {/* شريط اختيار نطاق الاستشارة */}
              <div className="flex items-center justify-between gap-2 bg-[#F5F3EE] dark:bg-[#1A2420] px-3.5 py-2 border-b border-[#E6E2D8] dark:border-[#2A3632] shrink-0">
                <div className="flex items-center gap-1.5 text-xs text-[#5F6B66] dark:text-[#8A9892] shrink-0">
                ...
                <select
                  ...
                  className="flex-1 rounded-lg border border-[#E6E2D8] dark:border-[#2A3632] bg-white dark:bg-[#17201D] px-2 py-1 text-xs text-[#1A1A1A] dark:text-white focus:outline-none truncate"
                >
              ...
              {/* منطقة المحادثة */}
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-3.5 space-y-3 custom-scrollbar min-h-0">
                {messages.map((msg, idx) => (
                  ...
                  >
                    <div
                      className={`max-w-[92%] sm:max-w-[90%] rounded-2xl p-3.5 text-sm leading-relaxed ${
                        msg.sender === "user"
                    ...
                    >
                      ...
                      {msg.citations && msg.citations.length > 0 && (
                        <div className="mt-2.5 pt-2 border-t border-[#E6E2D8] dark:border-[#2A3632]">
                          <span className="text-[10px] font-semibold text-[#5F6B66] dark:text-[#8A9892] block mb-1">
                            الأسانيد والأنظمة:
                          </span>
                          <div className="flex flex-wrap gap-1">
                            {msg.citations.map((cite, cIdx) => (
                              <span
                                key={cIdx}
                                className="inline-flex items-center gap-1 rounded bg-[#F5F3EE] dark:bg-[#202C28] px-1.5 py-0.5 text-[9px] text-[#123C32] dark:text-[#C9A961] border border-[#E6E2D8] dark:border-[#2A3632]"
                              >
              ...
              {messages.length <= 2 && !loading && (
                <div className="px-3 py-1.5 border-t border-[#E6E2D8] dark:border-[#2A3632] bg-white/80 dark:bg-[#17201D]/80 shrink-0">
                    ...
                    {GLOBAL_QUICK_PROMPTS.map((item, qIdx) => {
                      ...
                      return (
                        <button
                          ...
                          className="flex items-start gap-1 p-1.5 rounded-lg bg-[#FAF9F6] dark:bg-[#202C28] hover:bg-[#F0EDE6] dark:hover:bg-[#273530] text-right text-[10px] text-[#1A1A1A] dark:text-gray-200 border border-[#E6E2D8] dark:border-[#2A3632] transition-colors"
                        >
              ...
              {/* صندوق الإدخال — مراعاة المساحة الآمنة في الجوال */}
              <div className="p-3 border-t border-[#E6E2D8] dark:border-[#2A3632] bg-white dark:bg-[#17201D] shrink-0 pb-6 sm:pb-3">
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    handleSend();
                ...
                >
                  <textarea
                    ...
                    placeholder="اسأل بيان عن أي قضية أو مادة نظامية..."
                    rows={2}
                  ...
                  <button
                    ...
                    className="flex h-9 sm:h-10 w-9 sm:w-10 items-center justify-center rounded-xl bg-[#123C32] hover:bg-[#184E41] text-white disabled:opacity-50 transition-colors shadow-xs shrink-0"
                  >
