...
const FAILURE_CODES: Record<string, { label: string; advice: string; kind: FailureKind }> = {
  ...
  run_failed: {
    label: "محاولة إرسال سابقة لنفس المفتاح فشلت",
    advice:
      "ترفض خدمة البريد إعادة استخدام مفتاح التفرّد نفسه بعد فشل التشغيل. تُولَّد المنصة الآن مفتاحاً جديداً لكل محاولة، فأعد الإرسال من قائمة البريد.",
    kind: "retryable",
  },
  invalid_recipient: {
  ...
  smtp_auth_failed: {
    label: "بيانات دخول صندوق البريد مرفوضة",
    advice:
      "رفض خادم البريد اسم المستخدم أو كلمة المرور، فأُكملت الرسالة عبر خدمة البريد المُدارة. حدّث بيانات دخول الصندوق ثم شغّل «اختبار الاتصال» في تكامل البريد.",
    kind: "permanent",
  },
  smtp_connect_failed: {
    label: "تعذّر الاتصال بخادم البريد",
    advice: "راجع مضيف ومنفذ SMTP في تكامل البريد ثم أعد اختبار الاتصال.",
    kind: "retryable",
  },
  smtp_timeout: {
    label: "انتهت مهلة الاتصال بخادم البريد",
    advice: "الخادم لم يستجب في الوقت المحدد. أعد المحاولة، وإن تكرر فراجع إعداد الشبكة والمنفذ.",
    kind: "retryable",
  },
  smtp_not_configured: {
    label: "أسرار صندوق البريد غير مكتملة",
