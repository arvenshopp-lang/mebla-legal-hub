  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/50 p-4" onClick={onClose}>
      <div
        ...
        className={`w-full ${size === "lg" ? "max-w-3xl" : "max-w-lg"} rounded-2xl bg-white shadow-xl`}
        dir="rtl"
      >
        <div className="flex items-center justify-between border-b border-[#123C32]/10 px-5 py-4">
          <h3 className="text-base font-bold text-[#123C32]">{title}</h3>
          <button onClick={onClose} className="text-[#123C32]/60 hover:text-[#123C32]">✕</button>
        </div>
