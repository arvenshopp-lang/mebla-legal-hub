...
/** عميل يمرر رمز المستخدم المُتحقَّق منه ليعمل RLS بهويته تماماً. */
...
export const NOT_AUTHENTICATED = {
  content: [{ type: "text" as const, text: "غير مُصدّق: يجب ربط الحساب عبر تسجيل الدخول في منصة مِهلة." }],
  isError: true,
};

/** المكتب النشط للمستخدم الحالي (العضوية النشطة الأحدث). */
export async function activeOrganizationId(
  ...
  if (!data?.organization_id) throw new Error("لا يوجد مكتب نشط مرتبط بهذا الحساب.");
