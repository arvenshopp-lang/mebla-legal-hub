...
export function watermarkTileDataUrl(stamp: PrintStamp, opacity = 0.1): string {
  ...
  const text = lines
    .map(
      (line, index) =>
        `<text x="0" y="${index * lineHeight - blockHeight / 2 + lineHeight}" font-family="'Tajawal',sans-serif" font-size="11" fill="#123C32" text-anchor="middle">${escapeXml(line)}</text>`,
    )
    .join("");
...
export function classificationStampDataUrl(stamp: PrintStamp, opacity = 0.14): string | null {
  ...
  <g opacity="${opacity}" transform="translate(450 180) rotate(${ANGLE_DEG})">
    ...
    <text x="0" y="18" font-family="'Tajawal',sans-serif" font-size="68" font-weight="700" fill="#8A1F1F" letter-spacing="6" text-anchor="middle">${escapeXml(label)}</text>
  </g>
</svg>`;
  return toDataUrl(svg);
}
