...
export function OfficeLeadEmail({
  officeName,
  leadName,
  channel,
  serviceKey,
}: OfficeLeadEmailProps) {
  return (
    <Html lang="ar" dir="rtl">
      <Head />
      <Preview>طلب استشارة جديد من الصفحة العامة</Preview>
      <Body
        style={{ backgroundColor: "#F5F3EE", fontFamily: "Tahoma, Arial, sans-serif", margin: 0 }}
      >
        <Container
          style={{
        ...
        >
          ...
          <Section style={{ backgroundColor: "#F5F3EE", borderRadius: 8, padding: 16 }}>
            <Text style={{ color: "#123C32", fontSize: 14, margin: "0 0 6px" }}>
              مقدّم الطلب: {leadName}
            </Text>
            <Text style={{ color: "#123C32", fontSize: 14, margin: "0 0 6px" }}>
              المصدر: {CHANNELS[channel] ?? "زيارة مباشرة"}
            </Text>
            {serviceKey ? (
              <Text style={{ color: "#123C32", fontSize: 14, margin: 0 }}>
                الخدمة: {serviceLabel(serviceKey)}
              </Text>
            ) : null}
          </Section>
          <Text style={{ color: "#6b7671", fontSize: 13, lineHeight: 1.8, marginTop: 16 }}>
            بيانات التواصل وتفاصيل الطلب متاحة داخل منصة مِهلة في «الإعدادات ← الصفحة العامة ←
            العملاء المحتملون».
          </Text>
        </Container>
      </Body>
