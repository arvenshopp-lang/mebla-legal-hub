...
BEGIN
  ...
  -- إجماليات كل بند تُحسب وتُخزَّن حتى تظهر صحيحة في الفاتورة وملفات PDF.
  UPDATE public.platform_invoice_items i
     SET line_subtotal = round(greatest(i.quantity * i.unit_price - i.discount_amount, 0), 2),
         line_tax = CASE WHEN v_inv.tax_exempt THEN 0
                         ELSE round(greatest(i.quantity * i.unit_price - i.discount_amount, 0)
                                    * (i.tax_rate / 100.0), 2) END,
         line_total = round(greatest(i.quantity * i.unit_price - i.discount_amount, 0), 2)
...
DROP POLICY IF EXISTS "Platform staff read webhook endpoints" ON public.webhook_endpoints;
CREATE POLICY "Integration managers read webhook endpoints"
  ON public.webhook_endpoints FOR SELECT
  TO authenticated
  USING (private.has_platform_permission(auth.uid(), 'integrations.manage'));