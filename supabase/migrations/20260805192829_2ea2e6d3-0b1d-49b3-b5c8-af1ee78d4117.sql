...
BEGIN
  IF current_setting('role', true) IS DISTINCT FROM 'service_role'
     ...
     AND NOT (auth.uid() IS NOT NULL AND private.is_platform_staff(auth.uid())) THEN
    RAISE EXCEPTION 'FORBIDDEN' USING ERRCODE = 'P0001';
  END IF;

  PERFORM private.recalc_invoice(_invoice_id);
END;
$$;
...
REVOKE ALL ON FUNCTION public.admin_service_health() FROM PUBLIC;
