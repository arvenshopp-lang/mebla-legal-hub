...
check(
  "feature disabled + items=[] ⇒ لا يُعرض القسم",
  render({ enabled: false, computedAt: null, items: [] }) === "",
);

// 2 — الميزة مفعّلة لكن لا نتائج
check(
  "feature enabled + items=[] ⇒ لا يُعرض القسم",
  render({ enabled: true, computedAt: new Date().toISOString(), items: [] }) === "",
);

// 2b — معطّلة مع نتائج (Fail closed على مستوى العلم)
check(
  "feature disabled + items ⇒ لا يُعرض القسم",
  render({ enabled: false, computedAt: null, items: [office(1)] }) === "",
);
...
check(
  "شعار موجود ⇒ صورة بأبعاد ثابتة و alt",
  withLogo.includes("<img") && withLogo.includes('alt=""'),
);
...
check(
  "لا تسريب بيانات النزاهة",
  !leaky.includes("activeDays") && !leaky.includes("integrity") && !/\b41\b/.test(leaky),
);
check(
  "لا تسريب حالة الاشتراك أو الموافقة",
  !leaky.includes("subscriptionStatus") && !leaky.includes("publicOptIn"),
);
...
check(
  "القسم داخل الصفحة الرئيسية الفعلية",
  src.includes("<TopOfficesSection />") && src.includes("publicRankingQueryOptions"),
);
...
check(
  "لا منطق أهلية في الواجهة",
  !src.includes("PUBLIC_MINIMUM_SCORE") && !query.includes("integrity"),
);
