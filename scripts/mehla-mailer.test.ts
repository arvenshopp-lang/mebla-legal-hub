...
check("لا استدعاء sendLovableEmail أو sendAppEmail", !/sendLovableEmail|sendAppEmail/.test(MAILER_SRC));
...
check("قائمة الأسماء المستعارة خمسة بلا noreply", MEHLA_ALIAS_IDENTITIES.length === 5 && !MEHLA_ALIAS_IDENTITIES.includes("system" as never));
check("الأسماء المستعارة بلا بيانات دخول", MEHLA_ALIAS_IDENTITIES.every((id) => !mailboxHasOwnCredentials(identityAddress(id))));
...
check("فشل مُصنّف عند غيابه", !missing.ok && missing.errorCode === "mail_system_reply_to_not_configured");
...
check("معرّف الرسالة محفوظ في نتيجة الفشل", systemSend.messageId === "<notif-11111111-1111-1111-1111-111111111111@mehlalex.com>");
...
console.log("\n9) تصنيف الأخطاء");
check("رفض مستلم 5xx نهائي", classifyTransportFailure("smtp_rejected_recipient", 550) === "PERMANENT");
check("رفض مستلم 4xx قابل للإعادة", classifyTransportFailure("smtp_rejected_recipient", 451) === "RETRYABLE");
check("رفض محتوى 5xx نهائي", classifyTransportFailure("smtp_rejected_data", 552) === "PERMANENT");
check("رفض محتوى 4xx قابل للإعادة", classifyTransportFailure("smtp_rejected_data", 421) === "RETRYABLE");
check("رمز غير معروف قابل للإعادة", classifyTransportFailure("smtp_protocol_error", null) === "RETRYABLE");
check("مهلة قابلة للإعادة", classifyTransportFailure("smtp_timeout", null) === "RETRYABLE");
check("فشل اتصال قابل للإعادة", classifyTransportFailure("smtp_connect_failed", null, "ECONNRESET") === "RETRYABLE");
check("فشل TLS عطل إعداد", classifyTransportFailure("smtp_connect_failed", null, "certificate verify failed") === "SYSTEM_CONFIGURATION_FAILURE");
check("مصادقة فاشلة عطل إعداد", classifyTransportFailure("smtp_auth_failed", null) === "SYSTEM_CONFIGURATION_FAILURE");
check("رفض المُرسل عطل إعداد", classifyTransportFailure("smtp_rejected_sender", 553) === "SYSTEM_CONFIGURATION_FAILURE");
check("أسرار ناقصة عطل إعداد", classifyTransportFailure("smtp_not_configured", null) === "SYSTEM_CONFIGURATION_FAILURE");

console.log("\n10) حفظ رمز SMTP في مسارات الرفض");
const rcptBlock = SMTP_SRC.slice(SMTP_SRC.indexOf('code: "smtp_rejected_recipient",\n          message'));
...
check("لا ادّعاء تفرّد من المزوّد", !/idempotenc/i.test(MAILER_SRC.replace(/التفرّد يبقى على طبقة مِهلة/g, "")));

console.log("\n12) عنوان مستلم غير صالح يُرفض نهائياً قبل الاتصال");
process.env["MAIL_SYSTEM_REPLY_TO"] = "support@mehlalex.com";
const badTo = await sendMehlaEmail({ to: "not-an-email", identity: "system", subject: "س", html: "", text: "" });
check("رفض نهائي", !badTo.ok && badTo.errorClass === "PERMANENT" && badTo.errorCode === "smtp_rejected_recipient");
delete process.env["MAIL_SYSTEM_REPLY_TO"];
...
check("المرفقات البشرية كما هي", HOSTINGER_SRC.includes("attachments") || WORKSPACE_SRC.includes("buildAttachmentSection"));
check("لا استخدام للمُرسل الكنسي في مسار email_outbox", !WORKSPACE_SRC.includes("mehla-mailer"));
check("sendAppEmail لم يُحوَّل بعد", readFileSync("src/lib/email/app-email.server.ts", "utf8").includes("@lovable.dev"));
