...
## 2. الدوال الثماني عشرة المسموح استدعاؤها من `authenticated`
...
| `my_profile`                     | بيانات الملف الشخصي للمستخدم نفسه فقط (تشمل البريد والجوال). | `auth.uid()` إلزامي ويُقيّد الصف بالمستخدم | لا صلاحية إضافية؛ الصف واحد ومملوك للمستخدم                                  | `profiles`                                                                                   | أعمدة البريد والجوال محجوبة عن القراءة المباشرة، فتُقرأ بيانات المستخدم نفسه هنا فقط.     |
| `org_team_contacts`              | بيانات تواصل أعضاء المكتب (بريد/جوال) لمالك أو مدير المكتب. | `auth.uid()` إلزامي                | عضوية نشطة بدور `owner` أو `admin` في نفس المكتب                             | `organization_members`, `profiles`                                                           | يمنع رؤية بيانات تواصل الزملاء لكل الأعضاء، ويقصرها على الإدارة داخل نفس المكتب.          |
