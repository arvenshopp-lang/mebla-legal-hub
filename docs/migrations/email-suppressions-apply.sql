-- المصدر الكنسي المعتمد للتطبيق (Pre-Apply Fix A): ملفات supabase/migrations
-- تُدار بأداة الهجرة ولا تُحرَّر مباشرة، فيُقدَّم هذا النص حرفياً لأداة الهجرة
-- عند مراجعة التطبيق. الفرق الوحيد عن الملف السابق: إزالة المرجع الأجنبي
-- إلى auth.users من created_by و lifted_by (بلا SET NULL/CASCADE/RESTRICT).
...
CREATE TABLE public.email_suppressions (
  ...
  -- معرّف الفاعل بيانات تدقيق تاريخية بلا مرجع أجنبي: حذف مستخدم لا يحذف
  -- تاريخ الحجب ولا يعدّل حقائقه ولا يفشل، والقيمة لا تُعتبر دليلاً على بقاء
  -- المستخدم؛ تُقرأ للعرض التدقيقي فقط.
  created_by uuid,
